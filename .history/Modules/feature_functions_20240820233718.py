...
def is_recent_good(Table,Recent=5,Days=90):
    ...
    GoodStocks = Grpby.loc[Mask].index
    GoodStocks = []
    for Stock in GoodStocks:Result.append(Stock)
    GoodStocks = pd.DataFrame(Result)    
    GoodStocks.to_csv(WorkPath+'/good stocks.csv')
    return GoodStocks
